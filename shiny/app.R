...
# 2009-05-28;2009-07-16;2011-07-24;2014-05-13
# 2002-09-29;2010-11-13;2011-07-06;2015-08-15
           ...
           tabsetPanel(
             ...
             tabPanel("Download",
               wellPanel(textInput(inputId = "downloadBfmTableName", label = "Name for downloaded bfm table"),
                  ...
                  tags$hr(),
                  textInput(inputId = "downloadTsTableName", label = "Name for downloaded reference date table"),
             ...
             tabPanel("Bfm param.",
                      ...
                      selectInput("bfmFormula", label = "Formula",
                                  choices = list("trend", 
                                                 "trend + harmon", 
                                                 "harmon"),
                                  selected = "trend + harmon"),
                      numericInput("bfmOrder", label = "Harmonic order", value = 1),
                      ...
                      selectInput("bfmHistory", label = "History period",
                                  ...
                                  selected = "all"))),
             tabPanel("Bf param.",
                      wellPanel(selectInput("bfFormula", label = "Formula",
                                            choices = list("trend", 
                                                           "trend + harmon", 
                                                           "harmon"),
                                            selected = "trend + harmon"),
                                numericInput("bfOrder", label = "Harmonic order", value = 1),
                                sliderInput("bfMinSegment", label = "Minimum segment size (default 0.15)", min = 0.05, max = 0.95, step = 0.05, value = 0.15)))
           )),
    column(width = 5,
             ...
             tabsetPanel(
               tabPanel("Processed time series",
                        wellPanel(tags$p("bfastmonitor"),
                                  ...
                                  tags$h4(),
                                  tags$p("bfast"),
                                  plotOutput("bfPlot", height = 300, click = "bfPlotClick"),             # bf plot
                                  textOutput("bfPlotInfo"),
               ...
               tabPanel("Bfm table", tableOutput("bfmTable")),
               tabPanel("Bf table", tableOutput("bfTable")),
...
server <- function(input, output) {
  ...
  output$bfmPlot <- renderPlot({
    ...
    formula <- switch(input$bfmFormula,
    ...
    bfmOutput <- bfastmonitor(data, 
                             start = lubridate::decimal_date(input$bfmStartMon), 
                             formula = formula, 
                             order = input$bfmOrder, 
                             plot = FALSE, 
                             h = h, 
                             history = input$bfmHistory)
    
    bfmOutputPlot <- bfmPlot(list(bfmOutput),
              plotlabs = c(tsId), 
              ncols = 1, displayMagn = TRUE, displayResiduals = "monperiod") + 
  ...
  output$bfPlot <- renderPlot({
    ...
    # Reformat variable selected in UI
    formula <- switch(input$bfFormula,
                      "trend" = response ~ trend,
                      "trend + harmon" = response ~ trend + harmon,
                      "harmon" = response ~ harmon)
    
    # Time series preprocessing for subsequent regression modeling
    pp <- bfastpp(data, order = input$bfOrder)   
    
    # Run breakpoints(), which is the core algorithm of bfast
    bpOutput <- breakpoints(formula = formula, 
                            data = pp, 
                            h = input$bfMinSegment)
    
    # ggplot.bfastIR (below to fix)
    ggplotBfast(pp = pp, bpOutput = bpOutput, formula = formula, order = input$bfOrder) + # breakpts() is reactive that runs breakpoints() and return a list ?
      theme_bw() + labs(y = "NDMI", x = "Date", title = str_c("Id: ", tsId)) + scale_y_continuous(limits = c(-0.2, 0.6)) +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
  ...
  #   data <- tsDataProcessed()
  #   tsId <- as.character(rv$myDf$Id)    # just make sure Id is a character
  #   data <- data[[tsId]]                # This is a zoo ts object
  #   
  #   # Reformat variable selected in UI
  #   formula <- switch(input$bfmFormula,
  #                     "trend" = response ~ trend,
  ...
  #                             order = input$bfmOrder, 
  #                             plot = FALSE, 
  #                             h = input$bfmBandwidth, 
  #                             history = input$bfmHistory)
  #   
  #   allData <- bfmPredict(bfmOutput, type = "irregular", plotlabs = NULL)
  #   
  #   tsClicked <- nearPoints(allData[, c("time", "response")], 
  #                           input$bfmPlotClick,                              
  #                           xvar = "time", yvar = "response", maxpoints = 1)          
  # })
  ...
  # (b) bfm table (to change)  ----------------------------------------------------------
  
  # rvBfmTable <- reactiveValues()
  # rvBfmTable$myDf <- tibble(Id = character(0), 'Date' = numeric(0)) # Disturbance date
  # # Observer reacting to button "Register date" for bfmPlot
  # observeEvent(input$bfmRegisterDate, {
  #   tsId <- as.character(rv$myDf$Id)
  #   newRow <- tibble(Id = tsId, 'Date' = tsClicked()$date)                # change tsClicked()
  #   isolate(rvBfmTable$myDf <- rbind(rvBfmTable$myDf, newRow))
  # })
  # 
  # # Display the time series interpretation table in tabPanel 2
  # output$bfmTable <- renderTable({
  #   rvBfmTable$myDf
  # })
  # 
  # # Action button to download table
  # output$downloadBfmTable <- downloadHandler(
  #   filename = function() {
  #     paste(input$downloadBfmTableName, ".rds", sep = "")
  #   },
  #   content = function(file) {
  #     write_rds(rvBfmTable$myDf, file)
  #   }
  # )
