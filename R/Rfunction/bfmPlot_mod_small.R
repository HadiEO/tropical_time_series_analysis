bfmPlot_mod_small <- function (bfm, plotlabs = NULL, ncols = 1, rescale = 1, ylab = "response", 
                         ...
                         preCollection = TRUE) 
{
  ...
  xbks <-  as.Date(paste0(seq(1990,2015,by=5),'-01-01'))
  xbks.minor <- as.Date(paste0(seq(1988,2015,by=1),'-01-01'))

  
  # UPDATE: if show sensor
  if(showSensor) {
    # Need to add column "sensor" to allData
    # dataWithSensor <- extrNDMI
    # Todo later, move this outside fun i.e prepare a table of date and sensor, that table is input to fun
    dataWithSensor.tb <- tibble(date = decimal_date(index(dataWithSensor)), 
           sceneId = unlist(dimnames(dataWithSensor)[1]))
      
    dataWithSensor.tb <- dataWithSensor.tb %>% rowwise() %>%  
      mutate(sensor = getSceneinfo_mod(sourcefile = sceneId, preCollection = preCollection)[["sensor"]])    
      
    # For each original ts date, find the closest allData$time, assign sensor
    allData$sensor <- NA
    for(i in 1:NROW(allData)) {
      if(!is.na(allData[i,"response"])) {
        now.allData.date <- allData[i, "time"]
        closest.id <- which.min(abs(now.allData.date - dataWithSensor.tb$date))
        allData[i, "sensor"] <- dataWithSensor.tb[closest.id, "sensor"]
      }
    }
    
    p <- ggplot(data = allData, aes(x = as.Date(time2date(time)), y = response, col = sensor)) +     # HH: changed x = as.Date(time)
      geom_point(na.rm = TRUE) +
      scale_color_manual(values = c("dodgerblue", "magenta", "dark orange", "grey")) +
      labs(y = ylab) + scale_x_date(breaks = xbks,        # breaks = xbks
                                    minor_breaks = xbks.minor,
                                    date_labels = "%Y") +           # HH: changed scale_x_continuous(breaks = xbks)
